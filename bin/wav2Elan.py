...
# Masaru's account credentials
	speech_to_text = SpeechToTextV1(
		username= "21d70bd2-f53e-4b3a-a8f8-cfd19d24848d",
		password= "bJ1oRjtSdsXc"
	)
	print("sending:", filenameWav, "---", languageModel, "to server")
        
	with open(filenameWav, 'rb') as audio_file:
...
argv=sys.argv[1:]
try:
	opts, args = getopt.getopt(argv,"hd:f:l:j:a:",["dir=","file=","lang=","join=","Asr="])
...
for opt, arg in opts:
	if opt == '-h':
		options(sys.argv)
		sys.exit()
	elif opt in ("-d", "--dir"):  wavDir = arg
	elif opt in ("-f", "--file"): filenameWav = arg
	elif opt in ("-l", "--lang"): languageModel = arg
	elif opt in ("-a", "--Asr"):  
		convert2Asr(arg)
		sys.exit()
	elif opt in ("-j", "--join"):
