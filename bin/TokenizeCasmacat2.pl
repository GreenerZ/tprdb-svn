...
my $usage =
  "Tokenisation of Casmacat.v2 file: \n".
  "  -T in:  Translog XML <filename>\n".
  "  -D out: Write in {.src,.tgt} file\n".
  "Options:\n".
  "  -i use initialTargetText \n".
  "  -v verbose mode [0 ... ]\n".
  "  -h this help \n".
  "\n";

use vars qw ($opt_D $opt_T $opt_A $opt_i $opt_v $opt_h);
...
my $SourceLanguage = '';
my $TargetLanguage = '';
my $initialTargetText = 0;
my $SegID = {};

if (defined($opt_v)) {$Verbose = $opt_v;}
if (defined($opt_i)) {$initialTargetText=1;}
...
#printf STDERR "Translog: %s\n",  $_;
    ...
    if(/<Languages /i) {
      if(/Source="([^"]*)\"/i) {$SourceLanguage = $1; $SourceLanguage =~ s/-.*//;}
      if(/Target="([^"]*)\"/i) {$TargetLanguage = $1; $TargetLanguage =~ s/-.*//;}
    }

    elsif(/<SourceText[ >]/i) {$type =1; $seg=0;}
    elsif($initialTargetText == 0 && /<finalTargetText[ >]/i) {$type =3; $seg=0;}
    elsif($initialTargetText == 1 && /<initialTargetText[ >]/i) {$type =3; $seg=0;}
	
    if(($type == 1 || $type == 2 || $type == 3)  && (/<segment/i || length $inSegment > 0)) {
	if(/id="([^"]*)/i){ $segmentID = $1; }

	# save segment if it ends in this line
	if( /([^>]*)<\/seg/i){
	    $inSegment .= MSunescape($1);
	    $inSegment =~ s/\s*\n\s*/ /g;
...
#print STDERR "Source: $seg\t$id\t$text\n";
    ...
    if(/<\/SourceText/i) {$type =0; }
    if(/<\/initialTargetText/i) {$type =0; }
    if(/<\/finalTargetText/i) {$type =0; last;}
  }
  close(FILE);
  return $T;
}
...
sub PrintTag {
  ...
  printf FILE "<Text language=\"$language\" sent_segmenter=\"casmacat2\" >\n";
  my $word_id=1;
...
#printf STDERR "Tag: $f  $Tag->{$seg}{$f}{'end'}\t$Tag->{$seg}{$f}{'tok'}\n";
      ...
      $s .= "cur=\"$f\" id=\"$word_id\"";
      $word_id++;
      if(defined($Tag->{$seg}{$f}{'segId'})) { 
        my $seg_id = $Tag->{$seg}{$f}{'segId'}; 
        $s .= " segId=\"$seg_id\""; 
        $SegID->{$language}{$seg_id} ++;
      }
...
# printf STDERR "PrintAtag: $fn, $root $lang1, $lang2\n";  
  ...
  printf ATAG "<DTAGalign sent_alignment=\"casmacat2\" source=\"$lang1\" target=\"$lang2\" />\n";
  ...
  for my $seg (sort {$a<=>$b} keys %{$SegID->{$lang1}}) {
    if(!defined($SegID->{$lang2})) { printf STDERR "PrintAtag: $fn\tunaligned segment $seg\n"; next;}
